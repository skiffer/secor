...
public class SplitByNestedFieldMessageParserTest extends TestCase {
    ...
    public void testExtractTypeAndTimestamp() throws Exception {
        SplitByNestedFieldMessageParser jsonMessageParser = new SplitByNestedFieldMessageParser(mConfig);
    ...
    @Test
    public void testExtractPartitionsWithNestedJsonField() throws Exception {
        SecorConfig secorConfig = Mockito.mock(SecorConfig.class);
        Mockito.when(secorConfig.getMessageSplitFieldName()).thenReturn("type.value");
        Mockito.when(secorConfig.getMessageTimestampName()).thenReturn("timestamp");
        Mockito.when(secorConfig.getFinalizerDelaySeconds()).thenReturn(3600);
        Mockito.when(secorConfig.getTimeZone()).thenReturn(TimeZone.getTimeZone("UTC"));

        Mockito.when(TimestampedMessageParser.usingDateFormat(secorConfig)).thenReturn("yyyy-MM-dd");
        Mockito.when(TimestampedMessageParser.usingHourFormat(secorConfig)).thenReturn("HH");
        Mockito.when(TimestampedMessageParser.usingMinuteFormat(secorConfig)).thenReturn("mm");
        Mockito.when(TimestampedMessageParser.usingDatePrefix(secorConfig)).thenReturn("dt=");
        Mockito.when(TimestampedMessageParser.usingHourPrefix(secorConfig)).thenReturn("hr=");
        Mockito.when(TimestampedMessageParser.usingMinutePrefix(secorConfig)).thenReturn("min=");
        SplitByNestedFieldMessageParser jsonMessageParser = new SplitByNestedFieldMessageParser(secorConfig);
